...
**Note:** WebFetch converts HTML to markdown and strips `<head>` content, which removes JSON-LD blocks. Use `fetch_page.py` instead:
```bash
...
*4.* Deprecated/Changed Schemas to Flag
...
*5.* sameAs Strategy (CRITICAL for Entity Recognition)
