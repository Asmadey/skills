...
name: slop-monster
description: >-
  Очищает черновики и копирайтинг от признаков ИИ-генерации (AI slop), линтит клише и стилистические маркеры нейросетей, проводит многопроходный рерайт и очистку через альтернативную модель.
  Триггеры: использовать при запросах на хуманизацию текста, устранение ИИ-канцелярита, проверку на маркеры AI ("humanize this", "de-slop this", "does this sound like AI", "звучит ли это как ИИ", "убери налет нейросети", "сделай текст живым", "fix this copy", /slopmonster).
  Анти-триггеры: не активировать для чистого программного кода, JSON/YAML конфигураций, строгой технической документации API, а также если пользователь явно требует формальный академический или юридический слог без редакторской правки живой речи.
category: tooling
