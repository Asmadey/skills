#!/bin/bash
# Get Yandex OAuth token for LEGACY Wordstat API.
#
# DEPRECATED: Yandex no longer onboards new users to the legacy
# api.wordstat.yandex.net/v1 OAuth API. New users should get a
# Yandex Cloud Search API key instead and set YANDEX_WORDSTAT_API_KEY.
# See config/README.md for current instructions.
...
if [[ -z "$CLIENT_ID" ]]; then
    echo "=== LEGACY OAuth Token Setup (DEPRECATED) ==="
    echo ""
    echo "This flow is only useful if you already have an approved legacy"
    echo "Wordstat API application. New users should use Yandex Cloud Search API."
    echo ""
...
echo "=== Yandex OAuth Token Setup (legacy) ==="
echo ""
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    if grep -q "^YANDEX_WORDSTAT_TOKEN=" "$ENV_FILE"; then
        ...
        rm -f "$ENV_FILE.bak"
        echo "Updated YANDEX_WORDSTAT_TOKEN in: $ENV_FILE"
    else
        # Append
        echo "YANDEX_WORDSTAT_TOKEN=$TOKEN" >> "$ENV_FILE"
        echo "Added YANDEX_WORDSTAT_TOKEN to: $ENV_FILE"
    fi
else
    # Create new file
    echo "YANDEX_WORDSTAT_TOKEN=$TOKEN" > "$ENV_FILE"
