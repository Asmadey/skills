...
if [[ "$WORDSTAT_BACKEND" == "cloud" ]]; then
    if [[ "$REGION_TYPE" == "cities" ]]; then
        PARAMS="$PARAMS,\"regionType\":\"REGION_TYPE_CITIES\""
    elif [[ "$REGION_TYPE" == "regions" ]]; then
        PARAMS="$PARAMS,\"regionType\":\"REGION_TYPE_REGIONS\""
    fi
...
echo "$result" | grep -o '{"regionId":[0-9]*,"count":[0-9]*[^}]*}' | sort -t: -k3 -rn | head -30 | while IFS= read -r entry; do
    ...
    affinity=$(echo "$entry" | grep -o '"affinityIndex":[^,}]*' | sed 's/"affinityIndex"://' || echo "-")

    echo "| $region_id | $(format_number "$cnt") | $affinity |"
done
