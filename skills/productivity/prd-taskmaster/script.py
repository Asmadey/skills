...
Usage:
 ...
 script.py validate-prd --input  # Run 13 quality checks
 script.py calc-tasks --requirements  # Calculate recommended task count
 script.py gen-test-tasks --total  # Generate USER-TEST task specs
 script.py gen-scripts --output-dir  # Create 5 tracking scripts
 script.py backup-prd --input  # Timestamped backup
...
if len(sys.argv) < 3:
    print("Usage: track-time.py start|complete  [subtask_id]")
    sys.exit(1)
...
if len(sys.argv) < 2:
    print("Usage: execution-state.py status|start|complete|checkpoint ")
    sys.exit(1)
