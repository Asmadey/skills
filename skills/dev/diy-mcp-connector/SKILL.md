...
description: Автоматическое создание автономных локальных MCP-серверов (Model Context Protocol) для любых веб-приложений через анализ трафика (HAR / Chrome DevTools), авто-авторизацию сессий и генерацию инструментов.
category: integration
source: https://github.com/sshh12/claude-plugins/tree/main/plugins/diy-mcp-connector
version: 1.0.0
metadata:
  trigger: "Создание кастомного MCP-сервера под конкретный веб-сервис/сайт, подключение веб-приложений к Claude/IDE без браузерной автоматизации, анализ HAR-файлов трафика API, генерация коннекторов к API сервисов (WHOOP, Goodreads, банковские кабинеты, трекеры и т.д.)."
  anti_trigger: "Работа с уже существующими официальными SDK без потребности в MCP, ручная верстка UI, общие задачи по бэкенду без создания MCP-коннекторов."
---
