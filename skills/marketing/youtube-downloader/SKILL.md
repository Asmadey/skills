...
This downloads the video in best available quality as MP4 to `/mnt/user-data/outputs/` (or current directory if that doesn't exist).
...
Example:
```bash
python3 scripts/download_video.py "URL" -f webm
```
...
```bash
python3 scripts/download_video.py "URL" -a
```
...
```bash
python3 scripts/download_video.py "URL" -o /path/to/directory
```
...
```bash
python3 scripts/download_video.py "https://www.youtube.com/watch?v=dQw4w9WgXcQ" -q 1080p
```

2. Download audio only as MP3:
```bash
python3 scripts/download_video.py "https://www.youtube.com/watch?v=dQw4w9WgXcQ" -a
```

3. Download in 720p as WebM to custom directory:
```bash
python3 scripts/download_video.py "https://www.youtube.com/watch?v=dQw4w9WgXcQ" -q 720p -f webm -o /custom/path
```
