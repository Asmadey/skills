...
name: no-mistakes
description: >-
  Локальный гейт-прокси и пайплайн валидации изменений кода (review → test → docs → lint → push → PR → CI)
  в изолированном worktree перед отправкой в remote. Автоматически исправляет механические ошибки,
  согласует решения с пользователем и открывает чистый PR.
  Триггеры: использовать при запросах "no-mistakes", "безопасный пуш", "проверь и отправь",
  "gate changes", "валидируй перед пушем", /no-mistakes.
category: dev
