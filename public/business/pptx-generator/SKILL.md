...
**Note:** All skill resources are in `skills/pptx-generator/`. Always use Glob patterns starting with `skills/pptx-generator/` to find files.
...
```
Glob: skills/pptx-generator/brands/*/brand.json
```
...
```
Read: skills/pptx-generator/brands/template/README.md
Read: skills/pptx-generator/brands/template/brand.json
Read: skills/pptx-generator/brands/template/config.json
```

*2.* Gather Brand Information
...
*3.* Create Brand Files

1. **Create the brand folder:**
   ```bash
   mkdir -p skills/pptx-generator/brands/{brand-name}
   ```
...
```
Glob: skills/pptx-generator/brands/{brand-name}/*
```
...
2. **Read the brand configuration files:**
   ```
   Read: skills/pptx-generator/brands/{brand-name}/brand.json
   Read: skills/pptx-generator/brands/{brand-name}/config.json
   ```
...
3. **Read supporting markdown files for context:**
   ```
   Glob: skills/pptx-generator/brands/{brand-name}/*.md
   ```
...
*2.* Layout Discovery (READ ALL FRONTMATTERS)
...
```
Glob: skills/pptx-generator/cookbook/*.py
```
...
*3.* Slide Planning (ALWAYS DO THIS)
...
**Note: Follow these rules for ALL slide text.**
...
mkdir -p skills/pptx-generator/.tmp

# Write script to temp directory
# (create file at skills/pptx-generator/.tmp/gen.py)

# Execute
uv run --with python-pptx==1.0.2 python skills/pptx-generator/.tmp/gen.py

# MANDATORY: Clean up immediately after execution
rm skills/pptx-generator/.tmp/gen.py
```

**CRITICAL: Never create Python files in the repository root.** Always use heredoc or temp directory within the skill folder.

*6.* Quality Validation (MANDATORY)
...
*8.* Combine Batches (for multi-batch presentations)
...
*2.* Carousel Layout Discovery

**Discover carousel-specific layouts:**
```
Glob: skills/pptx-generator/cookbook/carousels/*.py
```
...
4. **Save to cookbook:**
   ```
   skills/pptx-generator/cookbook/{layout-name}-slide.py
   ```
...
1. **Find the layout:**
   ```
   Glob: skills/pptx-generator/cookbook/*{name}*.py
   ```
...
```bash
rm skills/pptx-generator/cookbook/{layout-name}.py
```
...
```bash
uv run skills/pptx-generator/generate-cookbook-preview.py
```
...
- [ ] **Read ALL cookbook layout frontmatters** (first 40 lines of each .py):
  - [ ] Glob all `skills/pptx-generator/cookbook/*.py`
...
This skill also includes legacy scripts for generating specific pitch decks (e.g., RentalLife) without using the cookbook logic.

**Available Scripts:**
- `python skills/pptx-generator/scripts/generate_pitch_deck_black.py`: Generates the RentalLife Pitch Deck using the Brand/Black theme.
- `python skills/pptx-generator/scripts/thumbnail.py <file.pptx>`: Creates image thumbnails of slides for visual inspection.
- `python skills/pptx-generator/scripts/office/unpack.py <file.pptx> <output_dir>`: Unpacks PPTX to XML structure (useful for low-level debugging).

Use these scripts when you need to regenerate the exact pitch deck structure defined in code, rather than creating new slides from scratch.
