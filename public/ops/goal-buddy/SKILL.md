...
name: goal-buddy
description: >-
  Автономный движок управления долгосрочными целями (Goal Management & Execution Engine). Формирует канбан-доску задач, обеспечивает декомпозицию намерений через Goal Oracle, ролевое распределение задач (Scout для разведки, Worker для реализации, Judge для проверки) и сохранение квитанций выполнения (durable receipts).
  Триггеры: использовать при запуске комплексных долгосрочных целей, проектировании пайплайна задач, подготовке бэклога ("goal", "goalbuddy", "goal-prep", "автономная цель", "разбей цель на задачи", "создай доску цели", /goal, /goalbuddy, /goal-prep).
  Анти-триггеры: не использовать для простых одношаговых правок ("исправь опечатку", "переименуй файл"), тривиальных команд или когда пользователь явно требует быстрого ответа без структурирования доски задач `state.yaml`.
category: automation
