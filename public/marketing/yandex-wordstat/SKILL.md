...
**Current backend (recommended):** `YANDEX_WORDSTAT_API_KEY` in `config/.env`.  
Uses Yandex Cloud Search API v2: `searchapi.api.cloud.yandex.net/v2/wordstat/*`.  
See `config/README.md` for API key setup instructions.

**Legacy backend (deprecated):** `YANDEX_WORDSTAT_TOKEN` in `config/.env`.  
Only works if you already have an approved `api.wordstat.yandex.net/v1` OAuth application.  
New Yandex accounts can no longer get access to the legacy endpoint.
